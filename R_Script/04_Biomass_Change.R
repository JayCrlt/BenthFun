...
for (i in 1:18) {
  ...
  Tile_cover[[i]] <- data.frame(Species = rep(Tile_cover[[i]]$Species, 4),
                                Tile    = rep(sheet_names_t[i], length(Tile_cover[[i]]$Species)*4),
...
# Fix positive ribbons at 1.2 if higher than 1.2
...
# Figure 2
Fig_2A = ggplot() + ggtitle("Fleshy communities") + 
...
Fig_2B = ggplot() + ggtitle("Mixed communities") + 
...
Fig_2C = ggplot() + ggtitle("Calcifying communities") + 
...
Fig_2D = ggplot(data_lolipop, aes(y = Biomass, x = x_location)) + 
...
Fig_2 <- Fig_2A + Fig_2B + Fig_2C + Fig_2D + plot_layout(guides = "collect", nrow = 1, widths = c(4,4,4,2))