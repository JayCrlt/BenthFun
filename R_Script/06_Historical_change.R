...
for (i in 1:18) {
  Tile_cover_T0[[i]] <- datasets_h[[i]] %>% column_to_rownames("Species") %>% select(matches("_t1_"), -matches("_back_")) %>%
    rowSums(na.rm = T) %>% data.frame() %>% rename("T0" = ".") %>% mutate(T0 = ((T0/25*100) / sum(T0/25*100)) * 100)
  Tile_cover_T1[[i]] <- datasets_h[[i]] %>% column_to_rownames("Species") %>% select(matches("_t3_"), -matches("_back_")) %>%
...
Tile_cover <- Tile_cover %>% left_join(corrected_names_h) %>% select(-c(Species, functional.group)) %>% 
  rename(Species = species_new) %>% filter(!grepl("dead", Species)) %>% 
  filter(Species != "tile") %>% left_join(Zone_pH_h)
...
Zone_pH_h       <- data_frame(Tile = sheet_names, pH = c(rep("ELOW", 6), rep("LOW", 6), rep("AMB", 6)))
Tile_cover    <- Tile_cover %>% left_join(Zone_pH_h) 
Tile_cover$pH <- factor(Tile_cover$pH, levels = c("ELOW", "LOW", "AMB"))

# Front cover (/50)
Tile_cover_T0 = vector("list", 18) ; Tile_cover_T1 = vector("list", 18) ; Tile_cover = vector("list", 18)
for (i in 1:18) {
  Tile_cover_T0[[i]] <- datasets_h[[i]] %>% column_to_rownames("Species") %>% select(matches("_t1_")) %>%
    rowSums(na.rm = T) %>% data.frame() %>% rename("T0" = ".") %>% mutate(T0 = ((T0/50*100)))
  Tile_cover_T1[[i]] <- datasets_h[[i]] %>% column_to_rownames("Species") %>% select(matches("_t3_")) %>%
...
# Transplants
Transplants_nut <- Nutrients_h %>% dplyr::filter(Experiment == "Historic") %>% group_by(Phase,pH) %>% group_split()
...
(Fig_4 <- FOR_AMB_HIS_plot + MIX_AMB_HIS_plot+ ENC_AMB_HIS_plot + 
    FOR_LOW_HIS_plot + MIX_LOW_HIS_plot + plot_spacer() +
    FOR_ELOW_HIS_plot + plot_spacer() + plot_spacer())

#ggsave(Fig_4, file = "Outputs/Figures/Historic/Fig_4", width = 20, height = 20, units = "cm", dpi = 300)
